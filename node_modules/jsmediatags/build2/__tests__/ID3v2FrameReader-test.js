...
describe("ID3v2FrameReader", function () {
  it("should read APIC tag", function () {
    ...
    var fileData = [].concat([0x00], // text encoding
    bin("image/jpeg"), [0x00], [0x03], // picture type - cover front
    bin("front cover image"), [0x00], [0x01, 0x02, 0x03, 0x04, 0x05] // image data
    );
  ...
  it("should read PCNT tag", function () {
    ...
    expect(frameReader).toBeDefined();
    var fileData = [].concat([0xaf, 0x19, 0x00, 0x00]);
    var fileReader = new ArrayFileReader(fileData);
    var data = frameReader(0, fileData.length, fileReader);
  ...
  describe("T* text tags", function () {
    ...
    describe("TXXX", function () {
      ...
      it("reads the description and value with iso-8859-1 charset", function () {
        ...
        0x00, // "ã" for description field
        0xe3, // null terminating byte
        0x00, // "ã" for URL field
        0xe3];
        var fileReader = new ArrayFileReader(fileData);
        var data = frameReader(0, fileData.length, fileReader);
        expect(data).toEqual(expected);
      ...
      it("reads the description and value with utf-16 charset", function () {
        var fileData = [].concat([0x01], // encoding
        [0xfe, 0xff, 0x00, 0xe3, 0x00, 0x00], 0xe3);
        var fileReader = new ArrayFileReader(fileData);
        var data = frameReader(0, fileData.length, fileReader);
        expect(data).toEqual(expected);
      });
      it("reads the description and value with utf-16be charset", function () {
        var fileData = [].concat([0x02], // encoding
        [0xff, 0xfe, 0xe3, 0x00, 0x00, 0x00], 0xe3);
        var fileReader = new ArrayFileReader(fileData);
        var data = frameReader(0, fileData.length, fileReader);
        expect(data).toEqual(expected);
      });
      it("reads the description and value with utf-8 charset", function () {
        var fileData = [].concat([0x03], // encoding
        [0xc3, 0xa3, 0x00], 0xe3);
        var fileReader = new ArrayFileReader(fileData);
        var data = frameReader(0, fileData.length, fileReader);
        expect(data).toEqual(expected);
  ...
  it("should read TCON tag", function () {
    ...
    expect(frameReader).toBeDefined();
    var fileData = [].concat([0x00], // encoding
    bin("(10)Eurodisc"));
    var fileReader = new ArrayFileReader(fileData);
    var data = frameReader(0, fileData.length, fileReader);
  ...
  it("should read CTOC tag", function () {
    ...
    expect(frameReader).toBeDefined();
    var fileData = [].concat(bin("ID1"), // ID
    [0x00], // null terminated
    [3], // toplevel/ordered bit
    [0x02], // entry count
    ...
    var fileReader = new ArrayFileReader(fileData);
    var data = frameReader(0, fileData.length, fileReader, null, {
      major: 3
    });
    expect(data).toEqual({
      id: "ID1",
  ...
  it("should ignore faulty MP3ext padding", function () {
    ...
    var fileData = [].concat( // Good tag
    bin("TPE1"), [0x00, 0x00, 0x00, 1 + artistName.length], // size
    [0x00, 0x00], // flags
    [0x00], // text encoding
    artistName, // content
    ...
    var tags = ID3v2FrameReader.readFrames(0, fileData.length, fileReader, id3header);
  ...
  it("should ignore undeclared padding", function () {
    var artistName = bin("Lead Artist");
    var fileData = [].concat(bin("TPE1"), [0x00, 0x00, 0x00, 1 + artistName.length], // size
    [0x00, 0x00], // flags
    [0x00], // text encoding
    artistName, // content
    // undeclared padding
  ...
  it("should read UFID tag", function () {
    ...
    expect(frameReader).toBeDefined();
    var fileData = [].concat(bin("http://www.id3.org/dummy/ufid.html"), [0x00], // owner identifier
